...
Here, we've explicitly set `type = "line"` to create a line plot. `plot2()` supports a wide range of plot types, and you can use either the full name (`"geom_line"`) or an abbreviation (`"line"`, `"l"`).
...
You can use any colour that the implemented `colour()` function can understand, such as colour names and HTML codes:
...
In this example, the `Rock Salt` font, a playful and hand-drawn style, is applied across the plot. The `text_factor` argument shows that it can be used to scale the text size of all plot elements, ensuring readability and aesthetic balance.
...
# Now every plot will use the Syne font by default
mtcars |> 
  plot2(mpg, hp, title = "Consistent Font Example")
```

In this example, all plots will now use the `Lobster` font, ensuring a consistent appearance across your visualisations.
