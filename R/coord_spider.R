...
#' Compared to `ggplot2::coord_polar()`, this coordinate system:
#' 
...
#'
#' Typical usage is to map a discrete variable (categories) to `x` (theta) and a continuous variable (scores) to `y` (radius), then draw a closed polygon with `geom_polygon()` and/or `geom_path()`.
#' 
...
#' @importFrom ggplot2 ggproto Coord calc_element
...
#' @format NULL
#' @export
CoordSpider <- ggproto("CoordSpider",
                       Coord,
                       ...
                       setup_panel_params = function(self, scale_x, scale_y, params = list()) {
                         ...
                         th_range <- th_scale$dimension(expand = c(0, 0))
                         r_range  <- r_scale$dimension(expand = c(0, 0))
                         
                         th_limits <- tryCatch(th_scale$get_limits(), error = function(e) NULL)
                         th_n <- if (!is.null(th_limits)) length(th_limits) else NA_integer_
                         
                         # radial breaks: try to use scale breaks, fallback to breaks_extended()
                         r_breaks <- tryCatch(r_scale$get_breaks(), error = function(e) NULL)
                         r_breaks <- r_breaks[is.finite(r_breaks)]
                         
                         if (length(r_breaks) == 0) {
                           r_breaks <- scales::breaks_extended(n = 5)(r_range)
                           r_breaks <- r_breaks[is.finite(r_breaks)]
                         }
                         
                         r_breaks <- r_breaks[r_breaks >= min(r_range) & r_breaks <= max(r_range)]
                         
                         # radial labels: try scale formatting, fallback to as.character()
                       ...
                       transform = function(self, data, panel_params) {
                         ...
                         if (!is.null(panel_params$theta_limits) &&
                           ...
                           # theta_val is typically 1..n after discrete scaling; be defensive.
                           theta_idx <- suppressWarnings(as.integer(theta_val))
                           
                           # If theta came through as a character/label (rare at this stage),
                           # fall back to matching to theta_limits.
                         ...
                         # radius scaling to [inner, outer]
                         r_scaled <- scales::rescale(
                           r_val,
                           from = panel_params$r_range,
                           to   = c(panel_params$inner_radius, panel_params$outer_radius)
                         )
                         
                         # theta to angles:
                         # - if discrete: assume positions 1..n and place at equal angles
                         # - else: linear map over theta_range
                         ...
                         # 12 o'clock reference: (sin, cos) like coord_radial()
                       ...
                       render_bg = function(self, panel_params, theme) {
                         # polygon rings + spokes, drawn in npc coordinates
                         ...
                         outer <- panel_params$outer_radius
                         
                         # map helpers
                         r_to_unit <- function(r) scales::rescale(
                           r, from = panel_params$r_range, to = c(panel_params$inner_radius, panel_params$outer_radius)
                         )
                         angle_k <- function(k) panel_params$start + panel_params$direction * (k - 1) / th_n * 2 * pi
                         
                         # spokes (end at outer radius)
                         spoke_x <- (outer * sin(vapply(seq_len(th_n), angle_k, numeric(1))) + 1) / 2
                         spoke_y <- (outer * cos(vapply(seq_len(th_n), angle_k, numeric(1))) + 1) / 2
                         ...
                         if (identical(grid_col$colour, "white") || identical(substr(grid_col$colour, 1, 7), "#FFFFFF")) {
                           col <- "grey75"
                         } else {
                           col <- grid_col$colour
                         }
                         ...
                         if (isTRUE(self$axis_labels) &&
                           ...
                           # endpoints on outer radius (not 1 anymore)
                           x0 <- (outer * sin(ang) + 1) / 2
                           y0 <- (outer * cos(ang) + 1) / 2
                           
                           # pad outward in mm along radial direction
                           ...
                           extra <- ifelse(abs(sin(ang)) < 1e-8 & cos(ang) > 0, 1, 0)  # top spoke
                         ...
                         children <- c(list(spokes), rings, theta_label_grobs, r_label_grobs)
...
#' @importFrom ggplot2 ggplot_add theme element_blank
#' @noRd
ggplot_add.CoordSpider <- function(object, plot, ...) {
  # this function is to remove the axis titles of a spider plot, which are always superfluous
  # set coord
  plot$coordinates <- object
  # apply default theme adjustments for spider charts
